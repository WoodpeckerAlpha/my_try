...
class TaskService {
	...
	async changeStatus(taskId, userId, newStatusId) {
		try {
			ValidateField.requiredFields({taskId, userId, newStatusId}, [
				"taskId",
				"userId",
				"newStatusId",
			]);
			...
			if (
				...
				!mongoose.Types.ObjectId.isValid(newStatusId)
			) {
				throw ApiError.BadRequest("Invalid ID format");
			}

			const [newStatusId, task] = await Promise.all([
				StatusService.validateExistingStatus(newStatusId),
